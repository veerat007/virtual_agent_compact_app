...
#  end
  ...
  action do |session|
    ...
    if result["birth_weekday"] == session["result"]
      if session["result_item"]["intention"] == "outstanding_balance"
        AnnounceOutstandinBalanceBlock
      elsif session["result_item"]["intention"] == "remaining_balance"
        AnnounceRemainingBalanceBlock
      else
